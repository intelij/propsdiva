...
class WC_Settings_Mailchimp_Discount extends WC_Settings_Page {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_admin_field_wpeditor', array( $this, 'wcmd_display_editor' ) );
		add_action( 'woocommerce_update_option_wpeditor', array( $this, 'wcmd_save_editor_val' ) );
		add_action( 'woocommerce_admin_field_uploader', array( $this, 'wcmd_display_uploader' ) );
		...
		add_action( 'wp_enqueue_scripts',  array( $this, 'wcmd_enque_admin_scripts' ) );
	}
	/**
	 * Get settings array
	 *
		...
		return apply_filters( 'woocommerce_' . $this->id . '_settings', array(
			...
			array(	'title' => __( 'Mailchimp Discount Settings', 'wcmd' ), 'type' => 'title','desc' => '', 'id' => 'signup_discount_title' ),
                ...
                array(
					...
					'desc' 			=> __( 'Add SOURCE merge tag for each signup to track the signups. You can then segment the list based on the source merge tag. To know more <a href="http://magnigenie.com/segment-mailchimp-list-using-signup-source/" target="_blank">click here</a>', 'wcmd' ),
					'type' 				=> 'checkbox',
					'id'				=> 'wcmd_source',
					'default' 			=> 'yes'											
				),
				...
				array(
					...
					'desc' 		  => __( 'Enter success message which will appear when user successfully subscribes to your mailchimp list.', 'wcmd' ),
					'desc_tip'	  =>  false
				),
	...
	*
	*/
	public function wcmd_save_editor_val( $value ) {
	...
	*/
	...
	public function wcmd_enque_admin_scripts() {
		if( is_admin() ) {
			...
			wp_enqueue_script( 'select2-jquery', plugins_url( 'assets/js/select2.min.js', WCMD_FILE ) , array( 'jquery' ), '1.0.0', true );
		}
	}
	...
	*/
	...
	?>
		<script type="text/javascript">
			...
			$( ':input.wcmd.wc-product-search' ).filter( ':not(.enhanced)' ).each( function() {
				var select2_args = {
