<?php
function lt_sc_rows($atts, $content = null) {
...
function lt_sc_cols($atts, $content = null) {	
	extract( shortcode_atts( array(
        'span' => '1/4',
        'animate' => '',
    ), $atts ) );
...
function lt_sc_grid($atts, $content = null) {
    extract( shortcode_atts( array(
        'padding' => '10px',
        'width' => '',
        'el_class' => ''
    ...
    $shortcode_id = rand();
    ob_start();
    if($padding){ $padding_w = $padding/2; ?>
...
add_shortcode('bery_banner_grid', 'lt_sc_grid');
add_shortcode('row', 'lt_sc_rows');
add_shortcode('col', 'lt_sc_cols');
