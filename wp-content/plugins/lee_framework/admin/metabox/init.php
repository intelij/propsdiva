...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		if ( file_exists(LEE_FRAMEWORK_PLUGIN_PATH.'/admin/metabox/helpers/'.$class_name.'.php'))
			include_once LEE_FRAMEWORK_PLUGIN_PATH . '/admin/metabox/helpers/'.$class_name.'.php';
	}
	...
	public function register_scripts() {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => esc_html__( 'Clear' , 'lee_framework' ),
					'defaultString' => esc_html__( 'Default' , 'lee_framework' ),
					'pick'          => esc_html__( 'Select Color' , 'lee_framework' ),
					'current'       => esc_html__( 'Current Color' , 'lee_framework' ),
				) );
			}
		} else {
		...
		wp_register_script( 'cmb-datepicker', LEE_FRAMEWORK_PLUGIN_URL . '/admin/metabox/js/jquery.datePicker.min.js' );
		wp_register_script( 'cmb-timepicker', LEE_FRAMEWORK_PLUGIN_URL . '/admin/metabox/js/jquery.timePicker.min.js' );
		wp_register_script( 'cmb-scripts', LEE_FRAMEWORK_PLUGIN_URL . '/admin/metabox/js/cmb'. $min .'.js', $scripts, self::CMB_VERSION );
		
		wp_register_script( 'lt-script-metabox', LEE_FRAMEWORK_PLUGIN_URL . '/admin/metabox/js/lt-script-metabox.js' );
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ UP ]&nbsp;',
			'down_arrow'      => '&nbsp;[ DOWN ]',
			'check_toggle'    => esc_html__( 'Select / Deselect All', 'lee_framework' ),
		) ) );

		wp_register_style( 'cmb-styles', LEE_FRAMEWORK_PLUGIN_URL . '/admin/metabox/style'. $min .'.css', $styles );
	...
	public function do_scripts( $hook ) {
		...
		wp_enqueue_script( 'lt-script-metabox' );
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$content_url = str_replace( $content_dir, WP_CONTENT_URL, LEE_FRAMEWORK_PLUGIN_PATH . '/includes/metabox' );
            ...
            $cmb_url = str_replace(
				...
				LEE_FRAMEWORK_PLUGIN_PATH . '/admin/metabox'
			);
		}
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save','lee_framework' ) );
