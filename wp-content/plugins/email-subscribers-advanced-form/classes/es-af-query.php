...
class es_af_query
{
	public static function es_af_count($id = 0)
	{
		...
		if($id > 0)
		{
			$sSql = $wpdb->prepare("SELECT COUNT(*) AS count FROM ". $prefix . ES_AF_TABLE . " WHERE es_af_plugin='es-af' and es_af_id = %d", array($id));
		}
		else
		{
			$sSql = "SELECT COUNT(*) AS count FROM ". $prefix . ES_AF_TABLE . " WHERE es_af_plugin='es-af'";
		}
	...
	public static function es_af_select($id = 0)
	{
		...
		if($id > 0)
		{
			$sSql = $wpdb->prepare("SELECT * FROM ". $prefix . ES_AF_TABLE . " where es_af_plugin='es-af' and es_af_id = %d", array($id));
		}
		else
		{
			$sSql = "SELECT * FROM ". $prefix . ES_AF_TABLE . " where es_af_plugin='es-af' order by es_af_id desc";
		}
		//echo $sSql;
	...
	public static function es_af_delete($id = 0)
	{
		global $wpdb;
		$prefix = $wpdb->prefix;
		$sSql = $wpdb->prepare("DELETE FROM ". $prefix . ES_AF_TABLE . " WHERE es_af_plugin='es-af' and es_af_id = %d LIMIT 1", $id);
		$wpdb->query($sSql);
		return true;
	}
	...
	public static function es_af_act($data = array(), $action = "ins")
	{
		...
		if($action == "ins")
		{
			$sql = $wpdb->prepare("INSERT INTO ". $prefix . ES_AF_TABLE ." 
		...
		elseif($action == "ups")
		{
			$sql = $wpdb->prepare("UPDATE ".$prefix . ES_AF_TABLE." SET `es_af_title` = %s, `es_af_desc` = %s, `es_af_name` = %s, `es_af_name_mand` = %s, `es_af_email` = %s, 
