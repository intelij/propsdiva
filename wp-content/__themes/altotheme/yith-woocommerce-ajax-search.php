...
<div class="row search-wrapper yith-ajaxsearchform-container <?php echo esc_attr($rand_id); ?>_container">
    <form method="get" id="yith-ajaxsearchform" action="<?php echo esc_url( home_url( '/'  ) ) ?>">
        <input class="search-input" type="search" value="<?php echo get_search_query() ?>" name="s" id="<?php echo esc_attr($rand_id); ?>_yith" placeholder="<?php esc_html_e('Search here','altotheme'); ?>&hellip;" />
    </form>
</div>
...
jQuery(function($){
    $('#<?php echo esc_attr($rand_id); ?>_yith').autocomplete({
        ...
        onSearchStart: function(){
            $('.<?php echo esc_attr($rand_id); ?>_container').append('<div class="please-wait"><span></span><span></span><span></span></div>');
        },
