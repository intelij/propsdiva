...
function lt_get_image( $attachment_id = 0, $width = null, $height = null, $crop = true, $post_id = null, $get_sizes = false ) {
	global $post;
	if (!$attachment_id) {
		if (!$post_id) {
			$post_id = $post->ID;
	...
	$image_url = lt_get_resized_url($attachment_id,$width, $height, $crop, $get_sizes);
	
	return $image_url;
}


function lt_get_resized_url($id,$width, $height, $crop, $get_sizes = false) {
	if ( function_exists("gd_info") && (($width >= 10) && ($height >= 10)) && (($width <= 1024) && ($height <= 1024)) ) {
		$vt_image = lt_vt_resize( $id, '', $width, $height, $crop );
		if ($vt_image)  {
			if ($get_sizes) {
				$image_url = $vt_image;
			} else {
...
if ( !function_exists('lt_vt_resize') ) {
	function lt_vt_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false) {
